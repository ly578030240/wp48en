...
if ( ! function_exists( 'hashone_breadcrumbs' ) ) :
...
function hashone_breadcrumbs() {
	...
	$sep            = '&rsaquo;'; // separator between crumbs
	...
	if (is_home() && !is_front_page()) {
		...
		if ($show_current) echo $sep . $before . get_the_title( $blog_page_id ) . $after;
		echo $wrap_after;
		...
		if ( is_category() ) {
			...
			if ( get_post_type() != 'post' ) {
				...
				if ( get_query_var('cpage') ) {
					echo $sep . sprintf($link, esc_url(get_permalink()), get_the_title()) . $sep . $before . sprintf($text['cpage'], get_query_var('cpage')) . $after;
				} else {
